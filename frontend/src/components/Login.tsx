...
import { UserLoginDto } from "../types/api";
...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.id) setMessage("Login successful!");
    ...
    <div className="auth-container flex items-center justify-center min-h-screen bg-gray-100">
      <form className="auth-form bg-white p-8 rounded shadow-md w-full max-w-md" onSubmit={handleSubmit}>
        ...
        <input
          ...
          required
        />
        <button className="auth-button w-full bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded transition-colors" type="submit">Login</button>
        <div className="auth-message mt-4 text-center text-red-500">{message}</div>
      </form>
    </div>
  );
};
